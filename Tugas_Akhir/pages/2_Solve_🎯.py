...
INPUT_FILES_PATH = os.path.join("Tugas_Akhir/app/input_files/Input_File.xlsx")
PIPELINE_FILE_PATH = os.path.join("Tugas_Akhir/app/pipeline_files/Pipeline_File.xlsx")
OUTPUT_FILE_PATH = os.path.join("Tugas_Akhir/app/output_files/solution_file")
...
def writeSoln(dur: int, target: float, crf: float, soln_arcs : Dict, soln_sources: Dict, soln_sinks: Dict, 
    ...
    with open(filename+'.csv', 'w', encoding='UTF8', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["Tugas_Akhir CO2 NETWORK OPTIMIZATION SOLUTION"])
